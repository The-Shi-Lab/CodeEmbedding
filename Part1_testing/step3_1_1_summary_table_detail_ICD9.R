...
write.csv(df,"~/EHRmapping/comparison/results/step3_1_1_ICD9_skat_vs_burden_pBonferroni.csv")

# df = uniqueSigBlock_fun(p.threshold=0, data1 = pvalues.burden.df, data2 = pvalues.skat.df)
# write.csv(df,"~/EHRmapping/comparison/results/step3_1_1_ICD9_skat_vs_burden_p0.csv")

# df = uniqueSigBlock_fun(p.threshold=0.05/length(all_blocks), data1 = pvalues.skat.df, data2 = pvalues.burden.df)
# write.csv(df,"~/EHRmapping/comparison/results/step3_2_1_ICD9_burden_vs_skat_pBonferroni.csv")

# df = uniqueSigBlock_fun(p.threshold=0, data1 = pvalues.burden.df, data2 = pvalues.skat.df)
# write.csv(df,"~/EHRmapping/comparison/results/step3_2_1_ICD9_burden_vs_skat_p0.csv")

### unique in SKAT, Bonferroni
data = read.csv("~/EHRmapping/comparison/results/step3_1_1_ICD9_skat_vs_burden_pBonferroni.csv")
data1 = data[grepl("subtotal",data$X),]

# UKB& MGI explanation
UKB_phecode = read.csv('/net/snowwhite/home/jiacong/EHRmapping/comparison/data/phecode_icd9_rolled.csv')
UKB_phecode = na.omit(UKB_phecode)
colnames(UKB_phecode)[1:3] = c("icd9","icd9.string","phecode")
MGI_phecode = read.csv('/net/snowwhite/home/jiacong/EHRmapping/comparison/data/phecode_icd9_rolled.csv')
colnames(MGI_phecode)[1:3] = c("icd9","icd9.string","phecode")

data2 = data1 %>%
  mutate(phenotype_WHO = UKB_phecode$Phenotype[match(data1$block,UKB_phecode$phecode)]) %>%
  mutate(phenotype_US = MGI_phecode$Phenotype[match(data1$block, MGI_phecode$phecode)]) %>%
...
data2
write.csv(data2, "~/EHRmapping/comparison/results/step3_1_1_ICD9_skat_vs_burden_pBonferroni_detail.csv")

# ### unique in SKAT, p=0
# data = read.csv("~/EHRmapping/comparison/results/step3_2_1_ICD9_skat_vs_burden_p0.csv")
# data1 = data[grepl("subtotal",data$X),]
...
# colnames(data2) = c("Phecode","Phenotype_WHO","Phenotype_US","p_skat","p_burden","UKB_count","MGI_count","UKB_normedCount","MGI_normedCount")
# data2
# write.csv(data2, "~/EHRmapping/comparison/results/step3_2_1_ICD9_skat_vs_burden_p0_detail.csv")


# ### unique in Burden
# data = read.csv("~/EHRmapping/mapping/results/blockCompare/ICD9_blockCompareAdjusted_uniqueBurden_raw.csv")
# data1 = data[grepl("subtotal",data$X),]
...
# colnames(data2) = c("Phecode","Phenotype_WHO","Phenotype_US","p_skat","p_burden","UKB_count","MGI_count","UKB_normedCount","MGI_normedCount")
# write.csv(data2, "~/EHRmapping/comparison/results/step3_2_1_ICD9block_pBon_uniqueBurden.csv")
