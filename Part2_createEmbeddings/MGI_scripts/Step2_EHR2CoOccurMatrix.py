...
parser = argparse.ArgumentParser(description='Create co-occurrence matrix for diagnostic code from UKB.')
parser.add_argument('-i', '--infile', help='Directory for input file (all numeric variables)')
parser.add_argument('-o', '--outfile', help='Directory for output file (code1, code2 and count)')
parser.add_argument('-w', '--windows', nargs='+', type=int, help='Enter the time windows (seperate it by whitespace)', default=[1, 7, 14, 30])
...
events = pd.read_csv(args.infile, header=0,sep="\t")

# determine patient ids for the chunk
chunk = args.chunk
tot_chuncks = args.tot_chunks
tot_pt = events['Deid_ID_num'].max()
chunk_per_pt = round(tot_pt / tot_chuncks)

min_pid = (chunk_per_pt*(chunk-1)+1)
max_pid = chunk*chunk_per_pt

if chunk == tot_chuncks: max_pid = tot_pt

print(min_pid)
print(max_pid)

events_perpt = events.loc[(events['Deid_ID_num'] >= min_pid) & (events['Deid_ID_num'] <= max_pid)]

# calculate cooccurrance matrix
