...
reformatUKB <- function(fields,dataCoding=F,onlyInfo=F){
	...
	ifelse(fields=="40006"|fields=="40013"|fields=="40005",fieldpattern <- "(.+)\\-.+\\..+",fieldpattern <- "f\\.(.+)\\..+\\..+")
	ifelse(fields=="40006"|fields=="40013"|fields=="40005",entrypattern <- ".+\\-(.+\\..+)",entrypattern <- "f\\..+\\.(.+\\..+)")
	...
	reformatted2	<- reformatted2 %>%dplyr::distinct(.keep_all = T)
